  ...
  var Banner = {
    ...
    init: function init() {
      ...
      var dom = domUtils.makeVarsFromIds(),
          txt1_split = new SplitText('#txt1-wrap', { type: ['lines', 'chars'] }),
      ...
      function frame0() {
        ...
        tl.staggerFrom(txt1_split.lines, 0.5, { cycle: { x: ['-=500', '+=500'] }, ease: Power4.easeOut }, 0.5, '+=0.5').to("#coin", 1, { y: "+=100", ease: Power4.easeIn }, "-=.3").to("#pig", 0.5, { autoAlpha: 0 }, '+=0.75').staggerTo(txt1_split.chars, 0.1, { autoAlpha: 0, x: '+=5' }, 0.01).from("#txt2-wrap", 0.5, { x: '-=500', ease: Power4.easeOut }).from("#hammer", .5, { autoAlpha: 0 }, "-=.3").to("#hammer", 0.5, { rotation: 25, repeat: 1, yoyo: true, ease: Power4.easeIn }, "+=1.5").staggerTo(txt2_split.chars, 0.1, { autoAlpha: 0, x: '+=5' }, 0.01).to("#hammer", .5, { autoAlpha: 0 }, "-=.5").call(rateCounter).staggerFrom(['#ef_txt', '#ef_txt2'], 0.5, { autoAlpha: 0 }, '+=1.5').from('#cta', 0.67, { autoAlpha: 0, ease: Power4.easeOut }).from("#tagline", 0.5, { autoAlpha: 0, ease: Power4.easeOut }).from('#legal', 0.5, { autoAlpha: 0 });
