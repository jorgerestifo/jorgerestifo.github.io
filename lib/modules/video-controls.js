...
export class VideoControls {
  constructor(source, width, height, options) {
    ...
    if (this.playsinline) { this.video.setAttribute('playsinline', ''); }
    if (this.autoplay) { this.video.setAttribute('autoplay', ''); }
    ...
    if (this.nativeContols) {
      ...
      if (this.posterTouch) { this.video.setAttribute('poster', this.posterTouch); } // touch specific poster
      if (this.startMuted) { this.video.setAttribute('muted', ''); }
      if (this.controls) { this.video.setAttribute('controls', ''); }
      ...
      if (this.poster) { this.video.setAttribute('poster', this.poster); } // default poster
    ...
    function injectStyles() {
      ...
      const iconPlay = {bp: '', bs: '', url: `${commonSVG}'M4 3v9l8-4.5-8-4.5z'/%3E%3C/svg%3E"`};
      const iconPause = {bp: '', bs: '', url: `${commonSVG}'M3 3h3v9h-3v-9zm6 0h3v9h-3v-9z'/%3E%3C/svg%3E"`};
      const iconUnmute = {bp: '', bs: '', url: `${commonSVG}'M1 5h3l3-3v11l-3-3h-3v-5zM9.3 5.7l-.7.7c.2.3.4.7.4 1.1s-.2.8-.4 1.1l.7.7c.4-.5.7-1.1.7-1.8s-.3-1.3-.7-1.8zm1.4-1.4l-.7.7c.6.6 1 1.5 1 2.5s-.4 1.8-1 2.5l.7.7c.8-.8 1.3-2 1.3-3.2s-.5-2.4-1.3-3.2zm1.4-1.4l-.7.7c1 1 1.6 2.4 1.6 3.9s-.6 2.9-1.6 3.9l.7.7c1.2-1.2 1.9-2.8 1.9-4.6s-.7-3.4-1.9-4.6z'/%3E%3C/svg%3E"`};
      const iconMute = {bp: '', bs: '', url: `${commonSVG}'M1 5h3l3-3v11l-3-3h-3v-5zm13.4.3l-.7-.7-2.2 2.1-2.1-2.1-.7.7 2.1 2.2-2.1 2.1.7.7 2.1-2.1 2.1 2.1.7-.7-2.1-2.2 2.2-2.1z'/%3E%3C/svg%3E"`};
      if (self.icons.play) { loadIcon(self.icons.play, iconPlay); }
      if (self.icons.pause) { loadIcon(self.icons.pause, iconPause); }
      if (self.icons.unmute) { loadIcon(self.icons.unmute, iconUnmute); }
      if (self.icons.mute) { loadIcon(self.icons.mute, iconMute); }
      let minMargin = 0;
      if (self.margin < 3) { minMargin = 3; }
    ...
    function createControls() {
      ...
      if (!self.startMuted) { onVolumeChange(); }
    ...
    function onSoundClick(e) {
      if (!self.propagation) { e.stopPropagation(); }
      if (self.video.muted) {
    ...
    if (this.nativeContols) {
      this.proxyShowControls = bool => {
        if (bool) {
          if (!this.video.hasAttribute('controls')) { this.video.setAttribute('controls', ''); }
        } else {
          if (this.video.hasAttribute('controls')) { this.video.removeAttribute('controls'); }
        }
      };
  ...
  createSource(source, videoElem) {
    if (typeof source === 'string') {
      ...
      for(let i = 0; i < source.length; i++) {
        ...
        if (/m4v/.test(ext)) { type = 'mp4'; }
        if (/ogv/.test(ext)) { type = 'ogg'; }
