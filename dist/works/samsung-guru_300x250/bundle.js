(function () {
  ...
  var Banner = {
    ...
    init: function init() {
      ...
      function frame0() {
        // TweenMax.set('#img',{transformOrigin:"50% 0%"})
        var tl = new TimelineMax({ onComplete: addRollover });
        dom.ad_content.classList.remove('invisible');

        tl.from('#headline-1', 0.1, { y: '+=50', autoAlpha: 0, ease: Sine.easeInOut }).to('#headline-1', 0.1, { y: '-=50', autoAlpha: 0, ease: Sine.easeInOut }, "+=2").from('#headline-2', 0.1, { y: '+=50', autoAlpha: 0, ease: Sine.easeInOut }).set('#headline-2', { autoAlpha: 0, ease: Sine.easeInOut }, "+=2").set('#img', { autoAlpha: 1 }).from('#img', 4, { scale: 1.6, ease: Sine.easeIn }, "-=0.5").set('#ad_content', { background: "white" }).set('#branding', { autoAlpha: 0 }).from('#arrows', 1, { y: 1600, ease: Sine.easeInOut }, "-=.5").set('#branding-ef', { autoAlpha: 1 });
      }
      ...
      function addRollover() {
        ...
        dom.ad_content.addEventListener('mouseenter', function () {
          TweenLite.to('#cta-up', 0.25, { autoAlpha: 0 });
          TweenLite.to('#cta-hover', 0.25, { autoAlpha: 1 });
        });

        dom.ad_content.addEventListener('mouseleave', function () {
          TweenLite.to('#cta-up', 0.25, { autoAlpha: 1 });
          TweenLite.to('#cta-hover', 0.25, { autoAlpha: 0 });
        });
      }
