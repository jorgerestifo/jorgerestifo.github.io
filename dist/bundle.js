  ...
  function init() {
    ...
    function loadAds() {
      for (var i = 0; i < urls.length; i++) {
        ...
        img.id = "img" + i; // img.style.transform= "scale(50%, 50%)";

        loadImg(img, div, ifrm); // t.style.display="none";

        div.className = "ad";
        div.addEventListener("mouseover", reloadAd); // div.appendChild(ifrm);
        // div.appendChild(t);
      ...
      function loadImg(img, div, ifrm) {
        img.addEventListener("load", function () {
          ...
          ifrm.style.height = img.height / 2 + "px"; // div.addEventListener("mouseover", reloadAd);

          div.appendChild(ifrm); // ifrm.style.display= "block";
    ...
    function reloadAd(e) {
      ...
      e.currentTarget.childNodes[0].contentWindow.location.reload(); // console.log(e.currentTarget);
